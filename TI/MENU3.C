 ...
 *--------------------------------------------------------------
   ...
   if (adapter != MONO) {          /* EGA / CGA */
       ...
       WINink         =  F_LYELLOW;
       WINpaper       =  B_BLUE;
       WINborder      =  F_WHITE|B_BLUE; /*BLACK;*/
...
/* ------------------- m_key -- TITLE ---------------------------------- UX  UY -- ink -- paper ---- border -- selected Item - command letter att --- flagLcom */
   ...
   initMenu(&bltMenu,  ALT|ALT_B,nullstr,    &mainMenu, VER, FIXM, NULL,    14, 1, MENUink, MENUpaper, MENUborder, MENUselected, MENUlcom, TRUE);
   initMenu(&compMenu, ALT|ALT_C,nullstr,    &mainMenu, VER, FIXM, NULL,    27, 1, MENUink, MENUpaper, MENUborder, MENUselected, MENUlcom, TRUE, LCOM_ATT);
   initMenu(&runMenu,  ALT|ALT_R,nullstr,    &mainMenu, VER, FIXM, NULL,    40, 1, MENUink, MENUpaper, MENUborder, MENUselected, MENUlcom, TRUE, LCOM_ATT);
   initMenu(&optMenu,  ALT|ALT_O,nullstr,    &mainMenu, VER, FIXM, NULL,    49, 1, MENUink, MENUpaper, MENUborder, MENUselected, MENUlcom, TRUE);
   initMenu(&editMenu, ALT|ALT_E,nullstr,    &mainMenu, VER, FIXM, NULL,    37, 1, MENUink, MENUpaper, MENUborder, MENUselected, MENUlcom, TRUE);
