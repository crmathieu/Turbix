...
main()
{
   ...
   ptr2 = (vl->operation == COMP ?  doCompStr : doAssStr);
   ...
   redirOn();
    ...
    /* enregistrer le nom de fichier … compiler
     * comme etant le futur EXE
     */
    strcpy(vl->exename, wkfile);

    typ = (vl->operation == COMP ? CTYPE : ATYPE);
    ...
    if (!vl->recomp) { /* afficher banniere "success" */
        strcpy(work, succStr);
...
int *diff;
{
      ...
      if (relation != LTYPE && relation != HTYPE) { /* COMPILE ou ASSEMBLE */
          if (relation == CTYPE) {
...
/*          if (strlen(c_i)) {
          ...
          strcpy(string,argf[2]);
          if (relation == ATYPE)
                string[strlen(argf[2])] = ';';

          string[strlen(argf[2])+1] = '\0';
          ...
          *(combuf+insert+noOverWriteOut+noOverWriteInc) = string;
          ...
          strcpy(string,argf[2]);         /* Nom absolu du fichier … compiler */
          string[strlen(argf[2])] = ';';
          string[strlen(argf[2])+1] = '\0';
          *(combuf+insert) = string;
          ...
          memset(work, '\0', 80);
          if (relation == CTYPE)
              strcpy(work2, compStr);
          else
              strcpy(work2,assStr);

          memset(work, '\0', 80);
          sprintf(work," %s ", ptr = strupr(ptr));
          strcat(work2, work);

          /* work2 contient "COMPILE file ou ASSEMBLE file
           * work contient le nom
           */
          winPopUp(wm, W_CLOSE);
          verify_window(wm, work2, ptr2, NMXMSG);

          *diff = TRUE;
          work2[i = strlen(work2)] = '\n';
          strcpy(vl->ferror_name, ptr);
          write(fmsg, work2, i+1);
